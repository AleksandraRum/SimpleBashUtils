...
{   
    ...
    {   
        if ((argv[i][0] == '-') && (argv[i][1] != 'e') && (strlen(argv[i]) == 2) && (argv[i][1] != 'f')) {
...
void open_file(char *template, char *name, char *flags, int *file_count){
    ...
    if ((fp == NULL) && (strchr(flags, 's') == NULL)) fprintf(stderr, "grep: %s: No such file or directory\n", name);
    fclose(fp); 
}
...
void change_file(char *flags, FILE *fp, regex_t reg, char *name, int *file_count, char *template)
{
    ...
    while (fgets(txt, LEN, fp) != NULL) {
        ...
        {
            if ((strchr(flags, 'c') == NULL)  && (strchr(flags, 'l') == NULL) && (strchr(flags, 'n') == NULL) && (strchr(flags, 'h') == NULL)) 
                {
                    if (strchr(flags, 'o') == NULL) printf("%s:%s", name, txt);
                    else printf("%s:%s\n", name, template);
                }
            if ((strchr(flags, 'c') == NULL)  && (strchr(flags, 'l') == NULL) && (strchr(flags, 'n') != NULL)) 
            {
                if (strchr(flags, 'o') == NULL) printf("%s:%d:%s", name, cur_line, txt);
                else printf("%s:%d:%s\n", name, cur_line, template);
            }
            if ((strchr(flags, 'c') == NULL)  && (strchr(flags, 'l') == NULL) && (strchr(flags, 'n') == NULL) && (strchr(flags, 'h') != NULL)) 
            {
                if (strchr(flags, 'o') == NULL) printf("%s",txt);
                else  printf("%s\n",template);
            }
        }
